...
import { useArgs } from "storybook/preview-api"; // 상태 동기화를 위해 필요
...
const meta: Meta<typeof FilterGroup> = {
  ...
  args: {
    title: "카테고리 선택",
    items: [
      { label: "의류", name: "clothes" },
      { label: "책", name: "books" },
      { label: "전자제품", name: "electronics" },
    ],
    value: [], // 초기값: 아무것도 선택 안 됨
    onChange: fn(), // 스파이 함수
  },
  argTypes: {
    title: {
...
export const Standard: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    const clothesInput = canvas.getByLabelText("의류", { selector: "input" });
    const booksInput = canvas.getByLabelText("책", { selector: "input" });
